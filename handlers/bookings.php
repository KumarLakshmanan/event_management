...
function handleCreate() {
    ...
    if (USE_DATABASE) {
        ...
        if ($packageData) {
            addNotification(
                'booking_created',
                "You've created a new booking for {$packageData['name']} on " . date('F d, Y', strtotime($eventDate)),
                $userId,
            ...
            addNotification(
                'booking_created',
                "{$userName} has created a new booking for {$packageData['name']} on " . date('F d, Y', strtotime($eventDate)),
                null, // System notification (for admins/managers)
                "../pages/bookings.php?id={$id}"
            );
        }
...
function handleConfirm() {
    ...
    if (USE_DATABASE) {
        ...
        foreach ($users as $u) {
            if ($u['id'] == $bookings[$bookingIndex]['user_id']) {
                $user = $u;
                break;
            }
        }
        ...
        foreach ($packages as $p) {
            if ($p['id'] == $bookings[$bookingIndex]['package_id']) {
                $package = $p;
                break;
            }
        }
        
        // Send confirmation email if user and package found
        if ($user && $package) {
            sendBookingConfirmationEmail($bookings[$bookingIndex], $user, $package);
        }
        ...
        if ($package) {
            addNotification(
                'booking_confirmed',
                "Your booking for {$package['name']} on " . date('F d, Y', strtotime($bookings[$bookingIndex]['event_date'])) . " has been confirmed{$discountMsg}.",
                $bookings[$bookingIndex]['user_id'],
                "../pages/bookings.php?id={$id}"
            );
        }
...
function handleCancel() {
    ...
    if (USE_DATABASE) {
        ...
        // Check permission - users can only cancel their own bookings
        if ($userRole === 'client' && $bookings[$bookingIndex]['user_id'] != $userId) {
            header('Content-Type: application/json');
            echo json_encode(['error' => 'Permission denied']);
            exit;
        }
        ...
        if ($package) {
            addNotification(
                'booking_cancelled',
                "Booking for {$package['name']} on " . date('F d, Y', strtotime($bookings[$bookingIndex]['event_date'])) . " has been cancelled.",
                $bookings[$bookingIndex]['user_id'],
                "../pages/bookings.php?id={$id}"
            );
        }
