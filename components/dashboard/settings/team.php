        ...
        <form action="" method="POST" id="shippingchargeFrom">
            <div class="row">
                <div class="form-group col-md-12">
                    <div class="p-2">
                        <label for="images">team user name</label>
                        <textarea class="form-control" id="teamUsernames"><?= getKeyValue('team_usernames', $settings) ?></textarea>
                    </div>
                </div>
                <div class="form-group col-md-12">
                    <div class="p-2">
                        <label for="images">team user position</label>
                        <textarea class="form-control" id="teamposition"><?= getKeyValue('team_positions', $settings) ?></textarea>
                    </div>
                </div>
                <div class="form-group col-md-12">
                    <div class="p-2">
...
<script>
    ...
    let _xUserData = {
        "baseURL": "<?= $baseUrl ?>",
        "auth": "<?= $_SESSION['token'] ?>",
        ".input-images": "<?= getKeyValue('team_images', $settings) ?>",
    };
    ...
    $("#saveButton").click(function(event) {
        ...
        swal({
            ...
            if (willDelete) {
                ...
                formData.append("team_usernames", $("#teamUsernames").val());
                formData.append("team_positions", $("#teamposition").val());
                formData.append("team_images", images);
                $.ajax({
                    url: "<?= $apiUrl ?>",
                    type: "POST",
                    data: formData,
