        ...
        <form action="" method="POST" id="shippingchargeFrom">
            <div class="row">
                <div class="form-group col-md-12">
                    <div class="p-2">
                        <label for="images">Bus Feature Name</label>
                        <textarea class="form-control" id="featureNames"><?= getKeyValue('feature_names', $settings) ?></textarea>
                    </div>
                </div>
                <div class="form-group col-md-12">
                    <div class="p-2">
                        <label for="images">Bus Feature Images</label>
...
<script>
    ...
    let _xUserData = {
        "baseURL": "<?= $baseUrl ?>",
        "auth": "<?= $_SESSION['token'] ?>",
        ".input-images": "<?= getKeyValue('feature_images', $settings) ?>",
    };
    ...
    $("#saveButton").click(function(event) {
        ...
        swal({
            ...
            if (willDelete) {
                ...
                formData.append("feature_names", $("#featureNames").val());
                formData.append("feature_images", images);
                $.ajax({
                    url: "<?= $apiUrl ?>",
                    type: "POST",
                    data: formData,
